...
class RRN(nn.Module):
    def __init__(self, scale, n_c, n_b):  # n_c: num channel, n_b: num block
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        data = torch.ones((N, 3, 2, H_lr, W_lr))
        out = out.cuda()
        feat = feat.cuda()
        data = data.cuda()
        print("data", data.shape, "feat", feat.shape, "out", out.shape)
        h, prediction = model(data, feat, out, True)
        print("h", h.shape)
